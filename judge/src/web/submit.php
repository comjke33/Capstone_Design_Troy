...
if (!isset($language_name) || $language >= count($language_name) || $language < 0) {
    $language = 0;
}
...
if (~$OJ_LANGMASK&(1<<$language)) {
  ...
  file_put_contents("/tmp/debug_log.txt", "error_type: ".json_encode($error_data)."\n", FILE_APPEND); 
  foreach ($error_data['stderrs'] as $stderr) {
    if (isset($stderr['message'])) {
      ...
      if(!empty($error_type)){
        $error_type = json_decode($error_type, true);
      }
      else{
