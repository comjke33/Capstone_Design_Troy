...
<div class="center-panel">
    ...
    <?php      
        ...
        function highlight_terms_with_tooltip($text) {
            ...
            // 단어 기준 정렬 (긴 단어 우선)
            uksort($term_map, function ($a, $b) {
                return mb_strlen($b) - mb_strlen($a);
            });
        
            foreach ($term_map as $term => $desc) {
                $tooltip = htmlspecialchars($desc, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                $tooltip = str_replace('&#10;', '<br>', $tooltip);
        
                // 이미 감싼 곳은 치환하지 않도록 negative lookahead 사용
                $text = preg_replace_callback(
                    '/(?<!<\/span>)(?<!term-tooltip">)' . preg_quote($term, '/') . '(?=[가-힣]{0,2})/u',
                    function ($m) use ($term, $tooltip) {
                        return '<span class="term-tooltip" data-html="true" data-content="' . $tooltip . '">' . $m[0] . '</span>';
                    },
                    $text
                );
            }
