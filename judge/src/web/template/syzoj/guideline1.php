...
<link href="https://cdn.jsdelivr.net/npm/pretendard@1.3.9/dist/web/pretendard.css" rel="stylesheet">
...
<div class="center-panel">
    ...
    <?php      
        function highlight_terms_with_tooltip($text) {
            $term_map = [
                "매개변수" => str_replace("\n", "<br>", "함수 괄호 안에 쓰는 변수예요. 함수가 일할 때 필요한 재료를 받는 그릇이라고 생각하세요!\n예: int sum(int a, int b) ← 여기서 a, b가 매개변수"),
                
                "초기화" => str_replace("\n", "<br>", "변수에 처음으로 값을 넣어주는 거예요. 꼭 해야 해요! 안 하면 이상한 값이 들어있을 수 있거든요.\n예: int x = 0; ← 이렇게 0을 넣어주는 게 초기화"),
                
                "선언" => str_replace("\n", "<br>", "컴퓨터에게 '이런 변수 쓸 거야!'라고 미리 알려주는 거예요.\n예: int age; ← age라는 정수 변수를 쓸 거라고 선언"),
                
                "변수" => str_replace("\n", "<br>", "값을 담아두는 상자예요. 이름표가 붙어있어서 나중에 찾아서 쓸 수 있어요.\n예: int score = 100; ← score라는 상자에 100을 담아둠"),
                
                "널" => str_replace("\n", "<br>", "'아무것도 없음'을 뜻해요. 포인터가 어디를 가리키지 않을 때 NULL이라고 써요.\n예: char* ptr = NULL; ← ptr이 아무것도 가리키지 않음"),
                
                "순회" => str_replace("\n", "<br>", "배열을 처음부터 끝까지 하나씩 둘러보는 거예요. 마치 책장 책을 하나씩 보는 것처럼!\n예: for(int i=0; i<5; i++) ← 배열을 하나씩 확인"),
                
                "인자" => str_replace("\n", "<br>", "함수를 부를 때 실제로 넣어주는 값이에요. 매개변수라는 그릇에 들어갈 실제 내용물!\n예: sum(3, 4); ← 여기서 3, 4가 인자"),
                
                "호출" => str_replace("\n", "<br>", "함수를 실행시키는 거예요. 함수 이름 뒤에 ()를 붙여서 써요.\n예: printf(\"안녕\"); ← printf 함수를 호출"),
                
                "삼항 연산자" => str_replace("\n", "<br>", "조건에 따라 둘 중 하나를 고르는 연산자예요. if문을 짧게 쓴 거라고 보면 돼요.\n형식: 조건 ? 참이면 이거 : 거짓이면 이거\n예: int max = (a > b) ? a : b;"),
                
                "인덱스" => str_replace("\n", "<br>", "배열에서 몇 번째인지 알려주는 번호예요. 중요한 건 0부터 시작한다는 거!\n예: arr[2] ← 배열의 3번째 요소 (0, 1, 2순서)"),
                
                "형식 지정자" => str_replace("\n", "<br>", "printf나 scanf에서 어떤 종류의 데이터를 출력하거나 입력받을지 알려주는 기호예요. %로 시작해요!\n예: %d(정수), %f(실수), %c(문자), %s(문자열)\nprintf(\"%d살\", age); ← %d 자리에 age 값이 들어감")
            ];
