...
<div class="center-panel">
    ...
    <?php      
        function highlight_terms_with_tooltip($text) {
            $term_map = [
                "매개변수" => "함수를 정의할 때 괄호 안에 선언하는 변수입니다.<br>외부에서 값을 넘겨받기 위해 사용됩니다.<br><strong>예:</strong> int sum(int a, int b); 에서 a, b가 매개변수입니다.",
            
                "초기화" => "변수를 처음 사용할 때 값을 설정하는 것을 말합니다.<br><strong>예:</strong> int x = 0; 에서 x는 0으로 초기화되었습니다.",
            
                "선언" => "변수나 함수를 처음으로 알려주는 작업입니다.<br><strong>예:</strong> int age; 또는 void greet();",
            
                "변수" => "값을 저장할 수 있는 이름이 붙은 공간입니다.<br><strong>예:</strong> int score = 100; 에서 score가 변수입니다.",
            
                "널" => "어떤 것도 가리키지 않는 상태를 나타냅니다.<br>C 언어에서는 NULL로 표현합니다.<br><strong>예:</strong> char *ptr = NULL;",
            
                "순회" => "배열이나 리스트처럼 여러 값을 하나씩 차례대로 접근하는 과정입니다.<br><strong>예:</strong> for (int i = 0; i < n; i++) 배열[i]를 확인합니다.",
            
                "인자" => "함수를 호출할 때 전달하는 실제 값입니다.<br><strong>예:</strong> sum(3, 4); 에서 3과 4가 인자입니다.",
            
                "호출" => "정의된 함수를 실행하는 것을 말합니다.<br><strong>예:</strong> printf(\"Hello\"); 는 printf 함수를 호출한 것입니다.",
            
                "삼항 연산자" => "조건에 따라 두 값 중 하나를 선택하는 연산자입니다.<br>형식: 조건 ? 참일 때 : 거짓일 때<br><strong>예:</strong> int max = (a > b) ? a : b;",
            
                "인덱스" => "배열 요소를 가리키기 위한 번호입니다. 0부터 시작합니다.<br><strong>예:</strong> arr[2]는 세 번째 요소입니다."
            ];
