...
<div class="center-panel">
    ...
    <?php      
        function highlight_terms_with_tooltip($text) {
            $term_map = [
                "선언" => "변수나 함수를 처음 정의하여 이름과 자료형을 지정하는 과정입니다.",
                "초기화" => "변수에 처음으로 값을 할당하여 유효한 상태로 만드는 작업입니다.",
                "변수" => "데이터를 저장하기 위해 이름을 붙인 메모리 공간입니다.",
                "널" => "값이 없음을 의미하는 특수한 상수로, 포인터가 아무 것도 가리키지 않을 때 사용됩니다."
            ];
        
            foreach ($term_map as $term => $desc) {
                $pattern = '/' . preg_quote($term, '/') . '/u';  // \b 제거
                $replacement = '<span class="term-tooltip" data-content="' . htmlspecialchars($desc, ENT_QUOTES, 'UTF-8') . '">' . $term . '</span>';
