...
<div class="center-panel">
    ...
    <?php      
        function highlight_terms_with_tooltip($text) {
            $term_map = [
                "매개변수" => str_replace("\n", "<br>", "함수를 정의할 때 괄호 안에 선언하는 변수로, 외부에서 값을 받아올 '그릇' 역할을 합니다. 함수가 작업을 수행하는 데 필요한 재료를 받는 통로입니다.\n예: int sum(int a, int b)에서 a와 b가 매개변수\n- 함수 정의 시에만 사용되는 이름\n- 실제 값은 함수 호출 시 인자로 전달받음"),
                
                "초기화" => str_replace("\n", "<br>", "변수를 처음 사용할 때 의미 있는 값을 설정하는 것입니다. 변수 선언 후 반드시 해야 하는 중요한 작업으로, 예측 가능한 프로그램을 만들기 위해 필수입니다.\n예: int x = 0; (선언과 동시에 초기화)\n- 초기화하지 않으면 쓰레기 값이 들어있을 수 있음\n- 선언 후 나중에 초기화도 가능: int y; y = 10;"),
                
                "선언" => str_replace("\n", "<br>", "변수나 함수를 사용하기 전에 컴퓨터에게 '이런 이름의 데이터 공간이 필요해요'라고 미리 알려주는 작업입니다. 메모리 공간을 예약하거나 함수의 존재를 알리는 역할을 합니다.\n예: int age; (변수 선언), void greet(); (함수 선언)\n- 선언만으로는 값이 설정되지 않음\n- 사용 전에 반드시 선언해야 함"),
                
                "변수" => str_replace("\n", "<br>", "값을 저장할 수 있는 이름이 붙은 메모리 공간입니다. 마치 이름표가 붙은 상자처럼, 숫자, 문자, 문자열 등 다양한 데이터를 넣었다가 꺼내서 사용할 수 있습니다.\n예: int score = 100; (score가 변수)\n- 데이터 타입에 따라 저장할 수 있는 값의 종류가 결정됨\n- 프로그램 실행 중 값을 변경할 수 있음"),
                
                "널" => str_replace("\n", "<br>", "포인터가 '아무것도 가리키지 않음'을 나타내는 특별한 값입니다. C에서 NULL로 표현되며, 포인터를 안전하게 사용하기 위해 초기화할 때 주로 사용됩니다.\n예: char* ptr = NULL;\n- NULL 포인터를 사용하기 전에 반드시 유효성 검사 필요\n- if(ptr != NULL) 형태로 체크 후 사용\n- NULL 포인터를 잘못 사용하면 프로그램 크래시 발생"),
                
                "순회" => str_replace("\n", "<br>", "배열이나 리스트처럼 여러 값을 가진 데이터 구조를 처음부터 끝까지 하나씩 차례대로 방문하여 확인하거나 처리하는 과정입니다.\n예: for (int i = 0; i < n; i++) printf(\"%d \", arr[i]);\n- 반복문(for, while)을 사용하여 구현\n- 배열의 모든 요소에 동일한 작업을 수행할 때 사용\n- 인덱스는 0부터 시작하여 (크기-1)까지"),
                
                "인자" => str_replace("\n", "<br>", "함수를 호출할 때 매개변수에 전달하는 실제 값입니다. 함수가 작업을 수행하는 데 필요한 실제 재료를 제공하는 역할을 합니다.\n예: sum(3, 4);에서 3과 4가 인자\n- 매개변수는 '그릇', 인자는 '내용물'이라고 생각\n- 함수 호출 시마다 다른 값을 전달 가능\n- 매개변수의 개수와 타입이 일치해야 함"),
                
                "호출" => str_replace("\n", "<br>", "미리 정의해둔 함수를 실제로 실행시키는 것입니다. 함수 이름 뒤에 괄호를 붙이고 필요한 인자를 넣어서 사용합니다.\n예: printf(\"Hello\"); (printf 함수 호출)\n- 함수명() 형태로 호출\n- 필요한 경우 괄호 안에 인자 전달\n- 함수 실행 후 원래 위치로 돌아옴\n- 반환값이 있는 경우 결과를 받을 수 있음"),
                
                "삼항 연산자" => str_replace("\n", "<br>", "조건에 따라 두 값 중 하나를 선택하는 연산자로, if-else 문을 한 줄로 축약한 형태입니다. 간단한 조건 판단을 코드를 짧게 표현할 때 유용합니다.\n형식: 조건 ? 참일 때 값 : 거짓일 때 값\n예: int max = (a > b) ? a : b;\n- 조건이 참이면 ? 다음 값, 거짓이면 : 다음 값 선택\n- 복잡한 조건보다는 간단한 선택에 사용 권장"),
                
                "인덱스" => str_replace("\n", "<br>", "배열의 각 요소를 구분하기 위한 번호로, C언어에서는 항상 0부터 시작합니다. 배열에서 특정 위치의 값에 접근할 때 사용하는 주소 개념입니다.\n예: arr[2]는 배열의 세 번째 요소\n- 첫 번째 요소는 인덱스 0\n- 배열 크기가 n이면 유효한 인덱스는 0 ~ (n-1)\n- 잘못된 인덱스 사용 시 예상치 못한 결과 발생 가능")
            ];
