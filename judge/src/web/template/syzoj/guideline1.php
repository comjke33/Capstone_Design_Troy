    ...
    <?php      
        ...
        function highlight_terms_with_tooltip($text) {
            ...
            foreach ($term_map as $term => $desc) {
                ...
                // 단어에만 직접 wrap (조사 있는 단어까지 포함하고 싶으면 정규식으로 교체 가능)
                // 단, 중복 치환 방지를 위해 기존에 이미 래핑된 span은 다시 치환하지 않도록
                $text = preg_replace_callback(
                    '/(?<!term-tooltip">)' . preg_quote($term, '/') . '(?=[가-힣]{0,2})/u',
                    function ($matches) use ($term, $with_br) {
                        return '<span class="term-tooltip" data-html="true" data-content="' . $with_br . '">' . $matches[0] . '</span>';
                    },
                    $text
                );
            }
