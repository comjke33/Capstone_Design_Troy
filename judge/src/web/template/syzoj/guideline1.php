            ...
            if ($block['type'] === 'text') {
                ...
                // depth == 1 이면 readonly + 정답 자동 표시
                 if ($depth == 1 && $has_correct_answer) {
                    ...
                    $html .= "<textarea id='ta_{$answer_index}' class='styled-textarea' data-index='{$answer_index}'  style='background-color: #D4EDDA; color: #155724; border: 1px solid #c3e6cb;'>{$answer_content}</textarea>";
                } else {
                    $escaped_line = htmlspecialchars($raw, ENT_QUOTES, 'UTF-8');
                    //코드라인 부분
