...
$output = str_replace(",", "&#44;", $_POST['output']);
...
// spj 값을 반드시 지정되게 설정 (기본값 '0')
$spj = isset($_POST['spj']) ? $_POST['spj'] : '0';
...
$pid = addproblem($title, $time_limit, $memory_limit, $description, $input, $output,
                  $sample_input, $sample_output, $hint, $source, $spj, $OJ_DATA);

// 태그 등록 로직: tag 테이블에 삽입하고, problem_tag 연관관계 설정
foreach ($tags_array as $tag_name) {
  $tag_name = trim($tag_name);
  if ($tag_name === '') continue;
  
  // tag 테이블에 존재 여부 확인 후 없으면 insert
  $sql = "SELECT tag_id FROM tag WHERE name = ? LIMIT 1";
  $result = pdo_query($sql, $tag_name);
  if (count($result) > 0) {
  ...
  // problem_tag 테이블에 연결
  $sql = "INSERT IGNORE INTO problem_tag(problem_id, tag_id) VALUES(?, ?)";
