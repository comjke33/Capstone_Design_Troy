...
$file_contents = preg_replace_callback($patterns['func_def'], function($matches) {
    return "<div class='code-block function' style='background-color: #e0f7fa; padding: 15px; margin-bottom: 20px; border-radius: 8px;'><h3>함수: {$matches[1]}</h3><p>" . nl2br($matches[2]) . "</p></div>";
}, $file_contents);

// 반복문 블록을 찾아서 출력
$file_contents = preg_replace_callback($patterns['rep'], function($matches) {
    return "<div class='code-block loop' style='background-color: #fce4ec; padding: 15px; margin-bottom: 20px; border-radius: 8px;'><h3>반복문: {$matches[1]}</h3><p>" . nl2br($matches[2]) . "</p></div>";
}, $file_contents);

// 조건문 블록을 찾아서 출력
$file_contents = preg_replace_callback($patterns['cond'], function($matches) {
    return "<div class='code-block conditional' style='background-color: #e8f5e9; padding: 15px; margin-bottom: 20px; border-radius: 8px;'><h3>조건문: {$matches[1]}</h3><p>" . nl2br($matches[2]) . "</p></div>";
}, $file_contents);

// self-contained 블록을 찾아서 출력
$file_contents = preg_replace_callback($patterns['self'], function($matches) {
    return "<div class='code-block self-block' style='background-color: #fff9c4; padding: 15px; margin-bottom: 20px; border-radius: 8px;'><h3>기본 문장: {$matches[1]}</h3><p>" . nl2br($matches[2]) . "</p></div>";
}, $file_contents);

// 구조체 블록을 찾아서 출력
$file_contents = preg_replace_callback($patterns['struct'], function($matches) {
    return "<div class='code-block struct' style='background-color: #ffecb3; padding: 15px; margin-bottom: 20px; border-radius: 8px;'><h3>구조체: {$matches[1]}</h3><p>" . nl2br($matches[2]) . "</p></div>";
}, $file_contents);
