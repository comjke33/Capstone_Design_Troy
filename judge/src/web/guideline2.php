...
$file_contents = preg_replace_callback($patterns['func_def'], function($matches) {
    return "<div class='code-block function' style='background-color: lightblue;'><h3>Function: {$matches[1]}</h3><p>" . nl2br(htmlspecialchars($matches[2])) . "</p></div>";
}, $file_contents);

// 반복문 블록을 찾아서 출력
$file_contents = preg_replace_callback($patterns['rep'], function($matches) {
    return "<div class='code-block loop' style='background-color: lightpink;'><h3>Loop: {$matches[1]}</h3><p>" . nl2br(htmlspecialchars($matches[2])) . "</p></div>";
}, $file_contents);

// 조건문 블록을 찾아서 출력
$file_contents = preg_replace_callback($patterns['cond'], function($matches) {
    return "<div class='code-block conditional' style='background-color: lightgreen;'><h3>Conditional: {$matches[1]}</h3><p>" . nl2br(htmlspecialchars($matches[2])) . "</p></div>";
}, $file_contents);

// self-contained 블록을 찾아서 출력
$file_contents = preg_replace_callback($patterns['self'], function($matches) {
    return "<div class='code-block self-block' style='background-color: lightyellow;'><h3>Self Block: {$matches[1]}</h3><p>" . nl2br(htmlspecialchars($matches[2])) . "</p></div>";
}, $file_contents);

// 구조체 블록을 찾아서 출력
$file_contents = preg_replace_callback($patterns['struct'], function($matches) {
    return "<div class='code-block struct' style='background-color: lightcoral;'><h3>Struct: {$matches[1]}</h3><p>" . nl2br(htmlspecialchars($matches[2])) . "</p></div>";
}, $file_contents);
