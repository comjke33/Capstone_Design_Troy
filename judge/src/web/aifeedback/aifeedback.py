...
def generate_hint(block_code, block_number, guideline, model_answer):
	...
	•	학생 코드에서 주어진 해당 영역(블록)만 판단 대상입니다. 예를들어 "* 선언하세요"라는 문장이 나오면 *의 입력,출력,연산은 포함되지않습니다.
	•	그 외의 코드(다른 함수, 선언, 문단, 헤더 등)는 절대 평가하지 마세요.
	•	헤더는 선언되어 있고, 작성된 부분 외 코드도 존재한다고 가정합니다.
	•	문제점이 없을 경우에도 다른 영역의 내용을 언급하지 마세요.

	1.	학생 코드가 가이드라인과 다른 부분을 간단히 지적하고 이유를 설명하세요.
	2.	가이드라인에 맞게 수정하려면 어떤 방향으로 수정해야 하는지 제안해주세요.
	3.	답변은 문제점, 수정방향, 학생이 제출한 코드의 알고리즘적인 내용 및 기능적의미에 대한 이야기, 왜 그 코드가 필요한지(코드의 역할)만 간단하고 호흡이 짧은 문장으로 언급해주시고 답을 알려주어선 안됩니다.
	4.	마크업은 하지 말아주세요.
	5.	문단 시작에는 번호를 붙이지 마세요.
	6.	학생 답이 맞을경우 "TROY OJ 사용 가이드” 위반한 것일 확률이 높습니다.
