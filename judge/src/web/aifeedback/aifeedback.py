...
def generate_hint(block_code, block_number, guideline, model_block, model_answer):
   ...
   다음은 학생이 제출한 코드 블럭입니다. 해당 블럭은 전체 프로그램의 특정 부분에 해당합니다.
    ...
    - `해당 블럭의 모범 코드`과 `학생이 제출한 코드`를 비교하여 코드가 다르다면, 어떻게 수정해야 할지를 한 줄로 지적하되 코드의 동작을 중점으로 **가능한 모든 핵심 차이점**을 지적하세요., . **단, '학생이 제출한 코드'가 '작성못함'인 경우에는 정답 코드 또는 '해당 블럭의 모범 코드'를 알려주지 말고 간접적인 힌트만 제공하세요.**
