...
def generate_hint(block_code, block_number, guideline, model_block, model_answer):
    ...
    2. 학생 코드가 가이드라인을 참조하여 해당 블럭의 모범 코드 와 다른 부분(문법 또는 로직)을 한 줄로 지적하세요. 또한 해당 블럭의 모범코드 영역이 이외의 코드는 지적하지말아주세요.
    3. 가이드라인에 맞게 어떻게 수정해야 할지 한 줄로 제안하세요.
    4. 이 가이드라인 블록이 전체 코드에서 어떤 역할을 하는지 한 줄로 설명하세요.
    5. 학생이 작성하지 못한경우 힌트를 알려주세요.
    6. 출력은 최대 3줄로 제한합니다. 그리고 번호를 붙이지말아주세요.
    7. 문장은 짧고 단정하게. 반복하지 마세요.
    8. 마크업(강조, 줄바꿈 등)은 하지 마세요.
