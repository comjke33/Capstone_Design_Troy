...
def generate_hint(block_code, block_number, guideline, model_answer):
    ...
    2. 가이드라인에 맞게 수정하려면 어떤 방향으로 수정해야 하는지 제안해주세요. 단, 가이드라인에 있는 내용만을 이용하여 답변해주세요. 다른 영역의 코드는 영향을 미치지않습니다. 헤더는 선언되었고 작성부분 이외의 코드들도 작성되었다고 가정합니다
    3. 답변은 문제점, 수정방향 2가지에 대하여만 간단하고 호흡이 짧은 문장으로 언급해주세요. 
    4. 마크업은 하지말아주세요.
    5. 글 문단 시작에 번호를 붙이지 말아주세요.
    6. 만약 학생이 답을 맞았을때에는 "TROY OJ 사용 가이드"부분을 위반했을 가능성이 있습니다. 이부분을 짚어주세요.
