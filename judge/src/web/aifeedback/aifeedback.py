...
def generate_hint(block_code, block_number, guideline, model_block, model_answer):
    ...
    prompt = f"""
    다음은 학생이 제출한 코드 블럭입니다. 해당 블럭은 전체 프로그램의 특정 부분에 해당합니다.

    - 아래의 `model_block`을 기준으로 학생 코드와 비교하여 한 줄로 틀린 부분을 지적하세요.
    - 비교는 반드시 `model_block`과만 하며, `model_answer` 전체 코드와 비교하지 마세요.
    - `block_code`가 정확히 일치한다면, "올바르게 작성되었습니다."라고 말해주세요.
    - 수정이 필요하다면, 가이드라인에 맞게 어떻게 수정할지 한 줄로 제안하세요.
    - 이 블럭이 전체 코드 흐름에서 어떤 역할을 하는지 한 줄로 설명해주세요.
    - 가능하다면 이 블럭이 **어떤 로직 또는 기능 흐름에 기여하는지** 간단히 말해주세요. (예: 입력 준비, 반복 조건 설정 등)
    - 학생이 아무 코드도 작성하지 않은 경우엔 힌트를 주세요.
    - 출력은 최대 3줄이며, 번호를 붙이지 마세요.
    - 문장은 짧고 단정하게. 반복하지 마세요.
    - 마크업(강조, 줄바꿈 등)은 하지 마세요.
