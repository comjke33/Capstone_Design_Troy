...
def generate_hint(block_code, block_number, guideline, model_block, model_answer):
   ...
   다음은 학생이 제출한 코드 블럭입니다. 해당 블럭은 전체 프로그램의 특정 부분에 해당합니다.
    ...
    - `model_block`과 `block_code`를 비교하여 **완전히 동일한 경우에만** "올바르게 작성되었습니다."라고 말하세요.
    - 코드가 다르다면 "올바르게 작성되었습니다."라는 말을 **절대 포함하지 마세요**.
    - 코드가 다르다면, `model_block`과 비교하여 다른 부분을 한 줄로 정확히 지적하세요.
    - 어떻게 수정해야 할지도 한 줄로 제안하세요.
