...
def generate_hint(block_code, block_number, guideline, model_answer):
    ...
    2. 가이드라인에 명시된 내용과 다르게 작성된 부분이 있으면 이를 간단하게 설명하고, 왜 잘못되었는지 이유를 알려주세요.
    3. 모범코드를 참고하여 가이드라인에 맞게 수정하려면 어떻게 해야 하는지 설명해주세요.
    4. 코드를 직접 제시하지 말고, 수정 방향과 힌트를 제시해주세요.
    5. 3줄 이내로 글을 간단하게 요약하여 답변해주세요.
    6. '작성못함'으로 답변이 되어 있는 경우 학생은 작성을 할 줄 모르는 경우입니다.
    7. 가이드라인 이외의 내용은 답안에 영향을 미치지않습니다.
