...
- 배열이 아닌 값을 인덱싱 연산자 []로 접근하려 한다면 오류가 발생합니다.
- 보통 다음과 같은 경우에 이 오류가 발생합니다:
    1. 정수형 변수에 []를 사용한 경우
    2. 포인터 변수의 초기화가 누락된 경우
    3. 포인터가 아닌 구조체 변수에 []를 사용한 경우
- 이러한 경우에 올바른 타입의 변수인지 확인하여 수정해야 합니다.
...
```c
int *p = arr;
printf("%d\n", p[2]); // OK: 포인터도 인덱싱 가능 (유효한 배열 참조 시)
```
...
```c
int *p;
p[0] = 3; // ❌ 오류: 초기화되지 않은 포인터를 배열처럼 사용
```
...
- C 언어에서는 배열과 포인터가 유사하게 동작하지만 엄밀히는 다릅니다.
- 예를 들어, int *p = NULL; 선언 후 p[0]을 접근하면 런타임 오류(segfault)가 발생할 수 있습니다.
    
    하지만 컴파일러는 타입만 보고 허용할 수 있으므로, 초기화 여부는 직접 확인해야 합니다.
    
- 배열이 아닌 변수(정수, 구조체 등)는 [] 연산자를 사용할 수 없습니다. 컴파일러는 이 경우 다음과 같은 오류를 출력합니다:

---

🧠 **추가 체크리스트**

- ✅ 인덱싱하려는 변수가 배열 또는 유효한 포인터인지 확인
- ✅ 포인터는 반드시 배열을 가리키도록 초기화
- ❌ 구조체나 기본형 변수에는 [] 사용 금지
- ❌ 인덱스 계산 시에도 실수로 정수형에 []를 붙이지 않도록 주의
