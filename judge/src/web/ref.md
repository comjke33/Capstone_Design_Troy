...
- 인자의 개수, 순서, 타입이 하나라도 일치하지 않으면 `"incompatible pointer type"`, `"too few arguments"` 등의 컴파일 오류가 발생하거나, 컴파일은 되더라도 논리적 오류나 런타임 크래시로 이어질 수 있습니다.
...
💡 보충 설명
- 함수 정의를 헷갈리지 않도록 함수 원형(prototype)을 명확히 선언하고, 코드 상단 또는 별도 헤더 파일로 관리하는 것이 좋습니다.

- 구조체를 인자로 넘길 때도 멤버 순서나 타입을 정확히 맞춰야 하며, 컴파일은 되더라도 값이 틀어질 수 있으니 주의해야 합니다.
...
- C 언어에서 break, continue, return, goto 등 **흐름 제어 키워드**는 반드시 정해진 위치에서만 사용할 수 있습니다.

- 예를 들어 break는 for, while, do-while, switch 문 내부에서만 사용할 수 있으며, 이 외의 위치에서 사용할 경우 컴파일 오류가 발생합니다.

- 또한 if와 else가 중첩될 경우, 중괄호 {}를 생략하면 **어느 if에 else가 연결되는지 모호**해져 “dangling else” 관련 경고가 발생할 수 있습니다.
...
```
int main() {
    break;  // 오류: 반복문이나 switch 문 외부에서 사용
    return 0;
}
```
...
if (x > 0)
    if (x % 2 == 0)
        ...
        printf("홀수입니다.\n");  // ⚠ 오류 아님, 그러나 어떤 if와 연결됐는지 모호 → 경고 발생 가능
```
...
- break, continue는 반복문이나 switch 블록 내부에서만 사용할 수 있습니다.
- return은 함수 내부에서만 사용할 수 있으며, 반환값 유무에 따라 void 또는 값 반환을 구분해야 합니다.
- dangling else 문제는 코드는 맞더라도 **의도와 다르게 실행될 위험**이 있어, 반드시 {}로 명확히 구분해야 합니다.
...
- 컴파일러 또는 런타임에서 출력한 오류 메시지가 현재 시스템의 오류 분류 기준에 일치하지 않을 경우, “기타 오류”로 처리됩니다.
